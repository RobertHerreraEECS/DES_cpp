...
/**
 * @brief flip byte order once total bytes have been split
 *
 */
...
void Round(char* L0,char* R0, char (*subKey)[8],char (*R)[4],char (*L)[4], int index) {
    ...
    memcpy(L[0],L0,4);
        ...
        //      if (index == 1) {
        //          printCharBinary(fOutput[index-1]);
        //      }
        // ri = li-1 ^ f(Ri-1,ki)
        for( i = 0; i < 4; i++) {
 ...
 /**
 * @brief straigh permutation function
 *
 */
...
/**
 * @brief expand and map permutated bytes
 *
 */
...
/**
 * @brief reverse the order of the substiution keys
 *
 */
...
/**
 * @brief generate the substitution keys
 *
 */
...
/**
 * @brief initial permutation for substitution
 *
 */
